...
public class RealmManagementPage extends WizardPage {
	...
	public RealmManagementPage() {
		...
		validationGroup = SwtValidationGroup.create(SwtValidationUI
				.createUI(this));
	}
	...
	private void createRealmToolbar(ViewForm viewForm) {
		...
		addRealmAction = new Action(Messages.realmManagementPage_add_action) {
			...
			public void run() {
				...
				for (int i = 1; i > 0; i++) {
					...
					if (!found) {
						String name = NLS
								.bind(Messages.realmManagementPage_newRealmNameTemplate,
										i);
						IAuthRealm newRealm = new NewAuthRealm(realmId,
								NLS.bind("<{0}>", name)); //$NON-NLS-1$
						realms.add(newRealm);
						realmViewer.refresh();
						realmViewer.setSelection(new StructuredSelection(
								newRealm));
		...
		addRealmAction.setImageDescriptor(PlatformUI.getWorkbench()
				.getSharedImages()
				.getImageDescriptor(ISharedImages.IMG_OBJ_ADD));
		addToolbarAction(toolBarManager, addRealmAction);

		removeRealmAction = new Action(
				Messages.realmManagementPage_remove_action) {
			@Override
			public void run() {
				IStructuredSelection selection = (IStructuredSelection) realmViewer
						.getSelection();
				if (!selection.isEmpty()) {
					final IAuthRealm authRealm = (IAuthRealm) selection
							.getFirstElement();
					...
					if (newUnsaved
						...
						if (newUnsaved) {
							...
							if (t == null) {
								...
								String message = NLS
										.bind(Messages.realmManagementPage_remove_error,
												t.getMessage());
								setMessage(message, IMessageProvider.ERROR);
								StatusManager
										.getManager()
										.handle(new Status(IStatus.ERROR,
												Activator.PLUGIN_ID, message, t));
							}
		...
		removeRealmAction
				.setToolTipText(Messages.realmManagementPage_remove_tooltip);
		removeRealmAction.setImageDescriptor(PlatformUI.getWorkbench()
				.getSharedImages()
		...
		reloadRegistryAction
				.setToolTipText(Messages.realmManagementPage_reload_tooltip);
	...
	private void createRealmViewer(ViewForm viewForm) {
		realmViewer = new TableViewer(viewForm, SWT.FULL_SELECTION
				| SWT.H_SCROLL | SWT.SINGLE | SWT.V_SCROLL);
				...
				.addSelectionChangedListener(new ISelectionChangedListener() {
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (selection.isEmpty()) {
							...
							IAuthRealm realm = (IAuthRealm) selection
									.getFirstElement();
							IAuthRealm previousRealm = realmManagementComposite
									.getRealm();
							if (realm != previousRealm) {
								...
								addRealmAction
										.setEnabled(!(realm instanceof NewAuthRealm));
							}
						}
					}
				});
	...
	private void createRealmDetails(Composite parent) {
		realmManagementComposite = new RealmManagementComposite(parent, null,
				validationGroup);
	}

	private void loadRealms() {
		realms = new ArrayList<IAuthRealm>(AuthFacade.getAuthRegistry()
				.getRealms());
	...
	private void updateSelection(String realmId) {
		if (realms.isEmpty()) {
			...
			realmViewer.setSelection(
					selectedRealm == null ? StructuredSelection.EMPTY
	...
	private boolean checkUnsavedChanges() {
		if (realmManagementComposite.isDirty()) {
			MessageDialog md = new MessageDialog(getShell(),
					Messages.realmManagementPage_realmChanged_title, null,
					...
					WARNING, new String[] {
							...
							Messages.realmManagementPage_realmChanged_cancel },
					1);
			if (md.open() == 1) {
				return true;
			}
